...
data class Exam(
    ...
    fun getInfoPairList(context: Context, includeUndefined: Boolean): List<Pair<String, String>> {
        ...
        if ((examNumber != UNDEFINED && examNumber.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.exam_number) to examNumber)
        if ((name != UNDEFINED && name.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.name) to name)
        if ((kind != UNDEFINED && kind.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.kind) to getLocalKindString(context))
        if ((category != UNDEFINED && category.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.category) to category)
        if ((bonus != UNDEFINED && bonus.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.bonus) to bonus)
        if ((malus != UNDEFINED && malus.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.malus) to malus)
        if ((ects != UNDEFINED && ects.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.ects) to ects)
        if ((sws != UNDEFINED && sws.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.sws) to sws)
        if ((semester != UNDEFINED && semester.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.semester) to semester)
        if ((tryCount != UNDEFINED && tryCount.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.attempt) to tryCount)
        if ((grade != UNDEFINED && grade.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.grade) to grade)
        if (state != State.UNDEFINED || includeUndefined) list.add(context.getString(R.string.state) to state.getLocalString(context))
        if ((comment != UNDEFINED && comment.isNotEmpty()) || includeUndefined) list.add(context.getString(R.string.comment) to comment)
