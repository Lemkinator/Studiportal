...
class DemoUseCase @Inject constructor(
    ...
    private suspend fun getDemoExams(addNewExam: Boolean = random.nextBoolean()): MutableList<Exam> {
        val newExams = examsRepository.getExams().toMutableList()
        if (!addNewExam) return newExams
        val exam = getRandomExam()
        val index = newExams.indexOfLast { it.category == exam.category }
        if (index == -1) newExams.add(exam) else newExams.add(index + 1, exam)
        return newExams
    }
    ...
    private val initialDemoExams = List(15) {
        when (it) {
            0 -> Exam.create(
                examNumber = getDemoExamName("Account"),
                name = "Demo Account",
